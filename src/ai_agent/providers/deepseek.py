...
class DeepSeekProvider:
    ...
    def _validate_messages(messages: list[Message]) -> None:
        # English: Tool messages need tool-call-specific mapping, which belongs to later phases.
        # 中文：Tool message 需要額外的 tool-call mapping，這部分留到後續 Tool Calling phase。
    ...
    def _parse_response(data: JsonResponse) -> ChatResponse:
        ...
        if not isinstance(choice, dict):
            raise InvalidRequestError(
                "DeepSeek response contained an invalid choice"
            )
        ...
        if not isinstance(content, str):
            raise InvalidRequestError(
                "DeepSeek response did not contain text content"
            )
